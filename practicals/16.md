...
- a phylogeny - but an ultrametric tree is much better
- either discrete or continuous trait(s) 

Here we will leverage an [Octopodiformes](https://en.wikipedia.org/wiki/Octopodiformes) phylogeny and a discrete classification of the maximum depth at which they have been observed:

- P - pelagic 
- E - euphotic, up to 100 meters in depth
- D1 - disphotic 1, from 100 up to 400 meters in depth
- D2 - disphotic 2, from 400 up to 800 meters in depth
...
Then, as seen in other instances, we have to find a model appropriate for our data. Also here, the central component is a rates matrix, not dissimilar from the Q matrix we have seen for amino acids / nucleotides models. 
...
Then we will assess how good their fit is, using the [Akaike weights](https://brianomeara.info/aic.html) - the bigger the better!
...
We can see that ER is the better fit!
...
```R
fitER <- ace(data,tree,model="ER",type="discrete")
```
...
```R     
plot.phylo(tree,lwd=7.5, edge.width = 7.5, label.offset = .2,
           show.tip.label = T,no.margin = TRUE, edge.color = "lightgray")
nodelabels(node=1:tree$Nnode+Ntip(tree),pie=fitER$lik.anc,cex=0.42, piecol = cols)
...
>So ... do Octopodiformes come from the photic zone ... or did they originate deep down 
...
Let's simulate a tree! Everybody should change these numbers a bit:

- the random seed (set to 42)
- the number of tips (set to 1000)
- the birth rate (set to 1.0)
- the death rate (set to 0.3)
...
tree<-NULL
while(is.null(tree)) 
  tree<-pbtree(n=1000,b=1,d=0.3,extant.only=T)
...
and then plot the resulting reconstructions! 
