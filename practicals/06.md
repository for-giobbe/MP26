...
A ready-to-run Docker environment All packages pre-installed, course data pre-cloned. No setup needed.
...
RStudio is ready with all packages and course data already inside

---

```
setwd("~/Desktop/MP/MP26")
library(Biostrings) 
library(msa)
library(ape)
library(seqinr)
...
and using the resulting distance matrix iteratively to construct a tree. Character-based methods include maximum parsimony (MP), 
...
However, you can still use other tree topology search strategy like NNI:
